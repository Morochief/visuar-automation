...
type VisuarCatalogData = Record<string, VisuarProduct[]>;

export const VisuarCatalog = () => {
    const [data, setData] = useState<VisuarCatalogData | null>(null);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch('/api/visuar_ac_data.json');
                const result = await response.json();
                setData(result);
            } catch (error) {
                console.error("Error fetching Visuar AC data:", error);
            } finally {
                setLoading(false);
            }
        };
    ...
    const downloadExcel = () => {
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'Catálogo Visuar');
        ...
        XLSX.writeFile(wb, `Catalogo_Visuar_${now}.xlsx`);
    };
    ...
    if (!data) {
        return (
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-8 text-center mt-6">
                <p className="text-slate-400">No se encontraron datos. Ejecuta el scraper The Visuar AC Scraper primero.</p>
            </div>
        );
    }
    ...
    return (
        <div className="mt-6">
            <div className="mb-6 flex flex-col md:flex-row md:justify-between md:items-center gap-4">
                <div>
                    <h2 className="text-2xl font-bold text-white">Catálogo Visuar: Aires Acondicionados</h2>
                ...
                <button
                    ...
                    className="flex items-center gap-2 px-5 py-2.5 rounded-lg font-medium shadow-lg transition-all text-white bg-emerald-600 hover:bg-emerald-500 shadow-emerald-900/20 hover:shadow-emerald-800/30 self-start"
                >
                    <Download size={18} />
                    Descargar Excel
                </button>
